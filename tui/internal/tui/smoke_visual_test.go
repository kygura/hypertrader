...
func TestSmokeFrame(t *testing.T) {
	...
	m.store.PutAssetCtx(metrics.AssetCtx{Coin: "BTC", MarkPrice: 95080, OraclePrice: 95050,
		Funding: 0.0001, OpenInterest: 1.6e6, DayVolume: 4.2e8, Time: now})
	m.upsertCandidate(verdict("BTC", metrics.ActionOpenLong, 0.71, "range reclaim; OI building with positive basis — squeeze fuel above 96k"))
	m.upsertCandidate(verdict("ETH", metrics.ActionHold, 0.35, "downtrend intact but funding neutral; no edge at mid-range"))
